...
// ─────────────────────────────────────────────────────────────────────────────
// NT API types (undocumented, but stable across Windows 7–11)
// ─────────────────────────────────────────────────────────────────────────────
...
/// Query current, minimum, and maximum resolutions (all in 100-ns units).
pub fn query_resolution() -> Result<TimerResolutionInfo> {
    let query_fn: FnNtQueryTimerResolution =
        get_ntdll_proc(b"NtQueryTimerResolution\0")?;
    ...
    if status != STATUS_SUCCESS {
        bail!("NtQueryTimerResolution returned NTSTATUS {:#010x}", status as u32);
    }

    Ok(TimerResolutionInfo { min_hns: min, max_hns: max, current_hns: current })
}

/// Set the timer resolution to `desired_hns` (100-ns units).
/// Returns the actual resolution that was applied.
pub fn set_resolution(desired_hns: u32) -> Result<u32> {
    let set_fn: FnNtSetTimerResolution =
        get_ntdll_proc(b"NtSetTimerResolution\0")?;

    let mut actual: u32 = 0;
    let status = unsafe { set_fn(desired_hns, 1 /* Set = TRUE */, &mut actual) };

    if status != STATUS_SUCCESS {
        bail!("NtSetTimerResolution returned NTSTATUS {:#010x}", status as u32);
    }
    Ok(actual)
}

/// Restore the timer resolution to the system default.
pub fn restore_resolution() -> Result<()> {
    let set_fn: FnNtSetTimerResolution =
        get_ntdll_proc(b"NtSetTimerResolution\0")?;

    let mut actual: u32 = 0;
    // SetResolution = FALSE (0) → restore default
    let status = unsafe { set_fn(TARGET_RESOLUTION_HNS, 0, &mut actual) };

    if status != STATUS_SUCCESS {
        bail!("NtSetTimerResolution (restore) returned NTSTATUS {:#010x}", status as u32);
    }
    Ok(())
}
...
impl TimerResolutionInfo {
    pub fn current_ms(&self) -> f64 { self.current_hns as f64 / 10_000.0 }
    pub fn min_ms(&self)     -> f64 { self.min_hns     as f64 / 10_000.0 }
    pub fn max_ms(&self)     -> f64 { self.max_hns     as f64 / 10_000.0 }
}
...
/// Query, then set 0.5ms. Returns before/after info.
pub fn apply() -> Result<TimerResult> {
    let info_before = query_resolution()?;

    // Clamp: don't request faster than hardware supports.
